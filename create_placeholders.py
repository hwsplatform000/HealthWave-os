      ...
      <Card className="flex flex-1 items-center justify-center border-dashed bg-transparent">
        <CardContent className="flex flex-col items-center justify-center p-12 text-center text-muted-foreground">
          ...
          <p className="mt-2 max-w-sm">
            This module is currently under development. Check back in a future update for {title.lower()} capabilities.
          </p>
        </CardContent>
      </Card>
    </div>
...
for name, title, desc, icon in pages:
    with open(f"artifacts/healthwave-os/src/pages/{name}.tsx", "w") as f:
        f.write(template.format(name=name, title=title, description=desc, icon=icon))
